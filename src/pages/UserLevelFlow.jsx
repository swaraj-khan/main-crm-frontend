...
const UserLevelFlow = () => {
  ...
  const [debouncedUserInfo, setDebouncedUserInfo] = useState('');
  ...
  useEffect(() => {
    const handler = setTimeout(() => {
      setDebouncedUserInfo(filterUserInfo);
      if (filterUserInfo) setCurrentPage(1);
    }, 500);
      ...
      if (debouncedUserInfo) {
        ...
        if (totalPages > 1) {
          ...
          for (let p = 2; p <= totalPages; p++) {
            promises.push(axios.get(`${API_URL}/crm/user-level`, { params: { ...params, page: p } }));
          }
